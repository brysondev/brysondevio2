...
var autores = false;
var fastSeekEnabled =
  typeof document.createElement("video").fastSeek != "undefined";
setResolution();
...
function check() {
  ...
  if (
    getEstFileSize(false) / 1024 >= 385 ||
    shortened ||
    document.getElementById("sampling").value == 1 ||
    width % 64 != 0 ||
    height % 64 != 0
  ) {
    ...
    if (!mipmapclick) document.getElementById("mipmapsCheck").checked = true;
...
setInterval(function () {
  ...
  if (filesize < 512)
    document.getElementById("filesizee").innerHTML =
      "Estimated file size: <span style='color:green'>" +
      filesize +
      "</span> [KB]";
  else
    document.getElementById("filesizee").innerHTML =
      "Estimated file size: <span style='color:red'>" +
      filesize +
      "</span> [KB]";
}, 200);
  ...
  for (var i = 0; i < files.length; i++) {
    if (files[i] && files[i].type.match("image.*")) {
      ...
      reader.onload = function (e) {
        ...
        if (this.fileType == "image/gif" && i == 0) {
          ...
          gif.load_raw(new Uint8Array(e.target.result), function (el) {
            ...
            updateHighestResolution(
              gif.get_hdr().width,
              gif.get_hdr().height,
              gif.get_frames().length
            );
            handleClipImport(
              gif.get_frames().length,
              false,
          ...
          this.fileType == "image/x-tga" ||
          this.fileType == "image/targa"
        ) {
          var tga = new TGA();
          tga.load(new Uint8Array(e.target.result));
          if (singleImageAnim) frameCount = tga.header.height / height;
          imagesLoaded += 1;
          ...
          img.onload = function () {
            if (singleImageAnim) frameCount = img.height / height;
            imagesLoaded += 1;
            frames[0].push(img);
            if (imagesLoaded == frameCount) {
      ...
      if (
        ...
        files[i].type == "image/x-tga" ||
        files[i].type == "image/targa"
      ) {
        reader.readAsArrayBuffer(files[i]);
        if (files[i].type == "image/gif") break;
      } else reader.readAsDataURL(files[i]);
      ...
      handleVideoLoadPre(files[i], function () {
        ...
        handleClipImport(this.duration, true, function (options) {
          ...
          updateHighestResolution(
            options.width,
            options.height,
            (options.end - options.start) / options.frametime
          );
          handleVideoLoad(video, frames[0], options, (progress) => {
            if (progress >= 1) {
              check();
...
function generateCanvas(ccanvas, cwidth, cheight) {
    ...
    for (var frame = 0; frame < frameCount; frame++) {
      ...
      canvas
        ...
        .drawImage(
          ...
          cwidth / 2 -
            (fimg.width * scale) / 2 +
            Math.floor((frame * height) / 32767) * cwidth,
          cheight / 2 -
            (fimg.height * scale) / 2 +
            cheight * (frame % getFrameRows()),
          fimg.width * scale,
          fimg.height * scale
        );
    }
  ...
  document.getElementById("filesizee").innerHTML =
    "Estimated file size: " + getEstFileSize() / 1024;
}

function createCanvas() {
  // put centered image on canvas
  ...
  for (
    var i = 2;
    width / i > 16 &&
    height / i > 16 /*&& (width/i) % 4 == 0 && (height/i) % 4 == 0*/;
    i *= 2
  ) {
    mipmapCount++;
    ...
    mipmapsHTML +=
      '<div id="inputWrapper' +
      mipmapCount +
      '"></div>\n<canvas class="mipmapElement" id="canvasMipmap' +
      mipmapCount +
      '"></canvas><br /><input type="file" id="files' +
      mipmapCount +
      '" name="files[]" accept="image/*,.tga,video/*" onchange="changeMipmap(event,' +
      mipmapCount +
      ')" multiple/>\n';
  }
  ...
  for (var i = 1; i <= mipmapCount; i++) {
    ...
    if (!frames[i])
      mipmaps[i]
        .getContext("2d")
        .drawImage(mipmaps[i - 1], 0, 0, mipwidth, mipheight);
    else generateCanvas(i, mipwidth, mipheight / frameCount);
  ...
  if (hasMipmaps)
    for (var i = getReducedMipmapCount(); i > 0; i--)
      convertPixels(
        i,
        width / Math.pow(2, i),
        getTotalImageHeight() / Math.pow(2, i)
      );
...
function changeMipmap(evt, mipmapNumber) {
    ...
    if (files[i] && files[i].type.match("image.*")) {
        ...
        if (this.fileType == "image/gif" && i == 0) {
          ...
          gif.load_raw(new Uint8Array(e.target.result), function (el) {
            handleClipImport(
              ...
              false,
              function (options) {
                handleGifLoad(gif, frames[mipmapNumber], options);
      ...
      handleVideoLoadPre(files[i], function () {
        ...
        handleClipImport(this.duration, true, function (options) {
          document.body.style.cursor = "url(img/aero_derpy_busy.ani), wait";
          reduceVideoSize(options, video);
          handleVideoLoad(video, frames[mipmapNumber], options, (progress) => {
            if (progress >= 1) {
              closeClipImport();
              loadMipmaps(mipmapNumber, cwidth, cheight);
              document.body.style.cursor = "auto";
...
function loadMipmaps(mipmapNumber, cwidth, cheight) {
  ...
  mipmaps[mipmapNumber]
    .getContext("2d")
    .clearRect(0, 0, cwidth, cheight * frameCount);
...
function setOutputType(el) {
  ...
  else document.getElementById("ditherBlock").style.display = "none";

  if (el.value == 13 || el.value == 15) {
    document.getElementById("dxtSettings").style.display = "block";
  } else document.getElementById("dxtSettings").style.display = "none";
  check();
  createCanvas();
}

function convertPixels(canvas, fwidth, fheight) {
  if (shortened) fwidth = fwidth - 4;
  ...
  var outimg;
  if (isdxt) {
    outimg = new Int32Array(
      (Math.ceil(fwidth / 4) * 4 * Math.ceil(fheight / 4) * 4) /
        (outputType == 13 ? 8 : 4)
    );
  ...
  for (var d = 0; d < getFrameColumns(); d++) {
    if (getFrameColumns() > 1)
      var pix = mipmaps[canvas]
        .getContext("2d")
        .getImageData(
          mipmaps[canvas].width / 2 / getFrameColumns() -
            fwidth / 2 +
            d * fwidth,
          0,
          fwidth,
          d == getFrameColumns() - 1
            ? (frameCount % getFrameRows()) * height
            : getFrameRows() * height
        );
    else
      var pix = mipmaps[canvas]
        .getContext("2d")
        .getImageData(
          mipmaps[canvas].width / 2 - fwidth / 2 + d * fwidth,
          0,
          fwidth,
          fheight
        );
    if (isdxt) {
      ...
      m_nCompressionSpeedAlpha =
        quality == 1 ? CMP_Speed_Fast : CMP_Speed_Normal;
      if (quality == 3) m_nRefinementSteps -= 1;
      ...
      for (var j = 0; j < pix.height / 4; j++) {
        // rows of blocks
        for (var i = 0; i < fwidth / 4; i++) {
          // columns of blocks
          //blockCount+=1;
          for (var y = 0; y < 4; y++) {
            for (var x = 0; x < 4; x++) {
              ...
              bufsrc[x + y * 4] =
                (pix.data[position + 3] << 24) +
                (pix.data[position] << 16) +
                (pix.data[position + 1] << 8) +
                pix.data[position + 2];
            }
          }
          if (outputType == 15) {
      ...
      reduceColors(
        ...
        5,
        8,
        document.getElementById("ditherCheck").checked
      );
      outputImage[canvas] = pix.data;
  ...
  mipmaps[canvas]
    .getContext("2d")
    .putImageData(pix, mipmaps[canvas].width / 2 - fwidth / 2, 0);
}

function createVTF() {
  var size = 0;
  if (outputType == 13) size = blockCount * 8;
  if (outputType == 15) size = blockCount * 16;
  else {
  ...
  var header = [
    ...
    0,
    0,
    12 + parseInt(document.getElementById("sampling").value),
    35 - hasMipmaps,
    0,
    0,
    frameCount,
    ...
    0,
    0,
    hasMipmaps ? getReducedMipmapCount() + 1 : 1,
    13,
    0,
    0,
    0,
...
function handleGifLoad(gif, cframes, options) {
  ...
  for (var j = options.start; j < options.end; j++) {
    ...
    var am =
      Math.ceil((time + gif.get_frames()[j].delay) / frametime) -
      Math.ceil(time / frametime);
    if (options.allFrames) am = 1;
    for (var k = 0; k < am; k++) {
      if (cframes == frames[0]) {
        imagesLoaded++;
        frameCount++;
      ...
      } else if (cframes.length >= frameCount) return;
      cframes.push(canvas);
    }
...
function handleVideoLoad(video, cframes, options, onprogress) {
  ...
  video.addEventListener("timeupdate", function () {
    ...
    canvas
      .getContext("2d")
      .drawImage(video, 0, 0, options.width, options.height);
    ...
    if (cframes == frames[0]) {
      ...
      if (!autores && !cancelPressed && getEstFileSize() / 1024 > 513) {
        if (
          window.confirm(
            "The remaining frames are skipped as they would exceed the frame limit. Press Cancel to preserve all frames"
          )
        ) {
          imagesLoaded--;
          frameCount--;
    ...
    cframes.push(canvas);
    onprogress(frameCount / framesc);
    if (fastSeekEnabled) video.fastSeek(video.currentTime + options.frametime);
    else video.currentTime += options.frametime;
  });
...
function reduceVideoSize(options, video) {
  ...
  var scale = Math.min(
    1,
    maxres / Math.max(video.videoWidth, video.videoHeight)
  );
  ...
  if (alpha1 > alpha2)
    switch (num) {
      ...
      case 1:
        return alpha2;
      case 2:
        return (6 * alpha1 + 1 * alpha2) / 7;
      case 3:
        return (5 * alpha1 + 2 * alpha2) / 7;
      case 4:
        return (4 * alpha1 + 3 * alpha2) / 7;
      case 5:
        return (3 * alpha1 + 4 * alpha2) / 7;
      case 6:
        return (2 * alpha1 + 5 * alpha2) / 7;
      case 7:
        return (1 * alpha1 + 6 * alpha2) / 7;
    }
  else
    switch (num) {
      case 0:
      ...
      case 1:
        return alpha2;
      case 2:
        return (4 * alpha1 + 1 * alpha2) / 5;
      case 3:
        return (3 * alpha1 + 2 * alpha2) / 5;
      case 4:
        return (2 * alpha1 + 3 * alpha2) / 5;
      case 5:
        return (1 * alpha1 + 4 * alpha2) / 5;
      case 6:
        return 0;
      case 7:
        return 255;
...
function reduceColors(data, rb, gb, bb, ab, dith) {
  ...
  for (var x = 0; x < data.width; x += 1) {
    for (var y = 0; y < data.height; y += 1) {
      ...
      var floor = [
        Math.round(Math.floor(d[pixel] / rm) * rm),
        Math.round(Math.floor(d[pixel + 1] / gm) * gm),
        Math.round(Math.floor(d[pixel + 2] / bm) * bm),
      ];
      var ceil = [
        Math.round(Math.ceil(d[pixel] / rm) * rm),
        Math.round(Math.ceil(d[pixel + 1] / gm) * gm),
        Math.round(Math.ceil(d[pixel + 2] / bm) * bm),
      ];
      ...
      if (dith) {
        ...
        for (var b = 0; b < 17; b += 1) {
          between.push(
            ...
            ] /*addColor(closest, multiplyColor(divideColor(closest2, 17), b))*/
          );
        }
...
function getLuminance(red, green, blue) {
  if (lumaPow)
    return (
      (0.2126 * red * red) / 255 +
      (0.7152 * green * green) / 255 +
      (0.0722 * blue * blue) / 255
    );
  else return 0.2126 * red + 0.7152 * green + 0.0722 * blue;
}
