...
m_fChannelWeights[2] = m_fBaseChannelWeights[2] = 0.082;
const nByteBitsMask = [0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff];
...
function Compress(bufferIn, bufferOut, pFeedbackProc, pUser1, pUser2) {
  ...
  if (
    bufferIn.GetWidth() != bufferOut.GetWidth() ||
  ...
  for (var j = 0; j < dwBlocksY; j++) {
    ...
    if (pFeedbackProc) {
      ...
      if (pFeedbackProc(fProgress, pUser1, pUser2)) return CE_Aborted;
    }
  }
...
function CalculateColourWeightings(block) {
  if (!m_bUseChannelWeighting) return NULL;
  ...
  if (m_bUseAdaptiveWeighting) {
    ...
    if (largest > 0) {
      ...
      medianG /= largest;
      medianB /= largest;
    } else medianR = medianG = medianB = 1.0;

    // Scale weightings back up to 1.0f
    var fWeightScale =
      1.0 /
      (m_fBaseChannelWeights[0] +
        m_fBaseChannelWeights[1] +
    ...
    m_fChannelWeights[0] =
      (m_fChannelWeights[0] * 3 * medianR + m_fChannelWeights[0]) * 0.25;
    m_fChannelWeights[1] =
      (m_fChannelWeights[1] * 3 * medianG + m_fChannelWeights[1]) * 0.25;
    m_fChannelWeights[2] =
      (m_fChannelWeights[2] * 3 * medianB + m_fChannelWeights[2]) * 0.25;
    fWeightScale =
      1.0 /
      (m_fChannelWeights[0] + m_fChannelWeights[1] + m_fChannelWeights[2]);
...
function CompressRGBBlock(
  ...
  if (bDXT1 && m_nCompressionSpeed == CMP_Speed_Normal) {
    ...
    var c0 = ConstructColour(
      nEndpointsl[RC * NUM_ENDPOINTS] >>> (8 - RG),
      nEndpointsl[GC * NUM_ENDPOINTS] >>> (8 - GG),
      nEndpointsl[BC * NUM_ENDPOINTS] >>> (8 - BG)
    );
    var c1 = ConstructColour(
      nEndpointsl[RC * NUM_ENDPOINTS + 1] >>> (8 - RG),
      nEndpointsl[GC * NUM_ENDPOINTS + 1] >>> (8 - GG),
      nEndpointsl[BC * NUM_ENDPOINTS + 1] >>> (8 - BG)
    );
    if ((!nMethod && c0 <= c1) || (nMethod && c0 > c1))
      compressedBlock[0] = c1 | (c0 << 16);
    else compressedBlock[0] = c0 | (c1 << 16);
    ...
    //uint
    var c0 = ConstructColour(
      nEndpoints[RC * NUM_ENDPOINTS] >>> (8 - RG),
      nEndpoints[GC * NUM_ENDPOINTS] >>> (8 - GG),
      nEndpoints[BC * NUM_ENDPOINTS] >>> (8 - BG)
    );
    var c1 = ConstructColour(
      nEndpoints[RC * NUM_ENDPOINTS + 1] >>> (8 - RG),
      nEndpoints[GC * NUM_ENDPOINTS + 1] >>> (8 - GG),
      nEndpoints[BC * NUM_ENDPOINTS + 1] >>> (8 - BG)
    );
    if (c0 <= c1) compressedBlock[0] = c1 | (c0 << 16);
    else compressedBlock[0] = c0 | (c1 << 16);
...
function CompRGBBlock(
  ...
  if (dwColors) {
    ...
    if (bHasAlpha && _bUseAlpha && !(dwNumPoints & 0x1)) return FLT_MAX;
    ...
    for (var i = 0; i < 3 * NUM_ENDPOINTS; i += 2)
      for (var j = 0; j < 2; j++) nEndpoints[i + j] = rsltC[i + j] & 255;
    ...
    if (_bUseAlpha && _Blk[i * 4 + AC] == 0) {
      ...
      if (_pfWeights)
        for (var r = 0; r < rmp_l; r++) {
          ...
          var distance =
            (_Blk[i * 4 + RC] - _Rmp[RC * 16 + r]) *
              (_Blk[i * 4 + RC] - _Rmp[RC * 16 + r]) *
              _pfWeights[0] +
            (_Blk[i * 4 + GC] - _Rmp[GC * 16 + r]) *
              (_Blk[i * 4 + GC] - _Rmp[GC * 16 + r]) *
              _pfWeights[1] +
            (_Blk[i * 4 + BC] - _Rmp[BC * 16 + r]) *
              (_Blk[i * 4 + BC] - _Rmp[BC * 16 + r]) *
              _pfWeights[2];
        ...
        for (var r = 0; r < rmp_l; r++) {
          ...
          var distance =
            (_Blk[i * 4 + RC] - _Rmp[RC * 16 + r]) *
              (_Blk[i * 4 + RC] - _Rmp[RC * 16 + r]) +
            (_Blk[i * 16 + GC] - _Rmp[GC * 16 + r]) *
              (_Blk[i * 4 + GC] - _Rmp[GC * 16 + r]) +
            (_Blk[i * 16 + BC] - _Rmp[BC * 16 + r]) *
              (_Blk[i * 4 + BC] - _Rmp[BC * 16 + r]);
      ...
      if (_bUseAlpha) blkPrv[i * 4 + AC] = 255;
      Err += shortest;
      // We have the index of the best cluster, so assign this in the block
      // Reorder indices to match correct DXTC ordering
      if (shortestIndex == dwNumPoints - 1) shortestIndex = 1;
      else if (shortestIndex) shortestIndex++;
...
function CompressRGBBlockX(
  ...
  for (i = 0; i < _UniqClrs * 4; i += 4)
    for (j = 0; j < 3; j++) Blk[i + j] = _BlkIn[i + j] / 255.0;
  ...
  if (!isDONE) {
    ...
    for (;;) {
      ...
      for (i = 0; i < _UniqClrs; i++) {
        Prj0[i] = Prj[i] =
          BlkSh[i * 4 + 0] * LineDir[0] +
          BlkSh[i * 4 + 1] * LineDir[1] +
          BlkSh[i * 4 + 2] * LineDir[2];

        PrjErr[i] =
          (BlkSh[i * 4 + 0] - LineDir[0] * Prj[i]) *
            (BlkSh[i * 4 + 0] - LineDir[0] * Prj[i]) +
          (BlkSh[i * 4 + 1] - LineDir[1] * Prj[i]) *
            (BlkSh[i * 4 + 1] - LineDir[1] * Prj[i]) +
          (BlkSh[i * 4 + 2] - LineDir[2] * Prj[i]) *
            (BlkSh[i * 4 + 2] - LineDir[2] * Prj[i]);
      ...
      if (Err + 0.001 < ErrG) {
        ...
        for (i = 0; i < _UniqClrs; i++) {
          ...
          if ((del = Prj0[i] - Pos[0]) <= 0) RmpIndxs[i] = 0.0;
          else if (Prj0[i] - Pos[1] >= 0) RmpIndxs[i] = dwNumPoints - 1;
          else RmpIndxs[i] = Math.floor((del + step_h) * rstep);
      ...
      } // We was not able to find anything better.  Drop dead.
      else break;
    }
...
function FindAxis(
  ...
  for (var i = 0; i < nNumColors; i++) {
    ...
    for (var j = 0; j < nDimensions; j++) {
      ...
      Crrl[j] +=
        _outBlk[i * 4 + j] * _outBlk[i * 4 + ((j + 1) % 3)] * _inpRpt[i];
    }
  }
...
function RampSrchW(
  ...
  for (var i = 0; i < _NmbClrs; i++) {
    ...
    if ((del = _Blck[i] - _min_ex) <= 0) v = _min_ex;
    else if (_Blck[i] - _max_ex >= 0) v = _max_ex;
    else v = Math.floor((del + step_h) * rstep) * step + _min_ex;
...
function MkRmpOnGrid(
  ...
  for (var j = 0; j < 3; j++) {
    for (var k = 0; k < 2; k++) {
      ...
      if (_RmpF[j * 2 + k] <= _Min) _RmpF[j * 2 + k] = _Min;
      else {
        _RmpF[j * 2 + k] +=
          Math.floor(128.0 / Fctrs1[j]) -
          Math.floor(_RmpF[j * 2 + k] / Fctrs1[j]);
...
function Refine3D(
  ...
  for (var i = 0; i < _NmrClrs; i++)
    for (var j = 0; j < 3; j++) Blk[i * 4 + j] = _Blk[i * 4 + j];
  ...
  for (var nJitterG0 = nRefineStart; nJitterG0 <= nRefineEnd; nJitterG0++) {
    InpRmp[GC * 2 + 0] = Math.min(
      Math.max(InpRmp0[GC * 2 + 0] + nJitterG0 * Fctrs[GC], 0.0),
      255.0
    );
    for (var nJitterG1 = nRefineStart; nJitterG1 <= nRefineEnd; nJitterG1++) {
      InpRmp[GC * 2 + 1] = Math.min(
        Math.max(InpRmp0[GC * 2 + 1] + nJitterG1 * Fctrs[GC], 0.0),
        255.0
      );
      Eq = MkWkRmpPts(Eq, WkRmpPts, InpRmp, nRedBits, nGreenBits, nBlueBits);
      ...
      for (var nJitterB0 = nRefineStart; nJitterB0 <= nRefineEnd; nJitterB0++) {
        InpRmp[BC * 2 + 0] = Math.min(
          Math.max(InpRmp0[BC * 2 + 0] + nJitterB0 * Fctrs[BC], 0.0),
          255.0
        );
        for (
          var nJitterB1 = nRefineStart;
          nJitterB1 <= nRefineEnd;
          nJitterB1++
        ) {
          InpRmp[BC * 2 + 1] = Math.min(
            Math.max(InpRmp0[BC * 2 + 1] + nJitterB1 * Fctrs[BC], 0.0),
            255.0
          );
          Eq = MkWkRmpPts(
            Eq,
          ...
          for (var i = 0; i < _NmrClrs; i++) {
            for (var r = 0; r < dwNumPoints; r++) {
              ...
              RmpErr[r * 64 + i] =
                RmpErrG[r * 64 + i] + DistB * DistB * fWeightBlue;
            }
          }

          for (
            var nJitterR0 = nRefineStart;
            nJitterR0 <= nRefineEnd;
            nJitterR0++
          ) {
            InpRmp[RC * 2 + 0] = Math.min(
              Math.max(InpRmp0[RC * 2 + 0] + nJitterR0 * Fctrs[RC], 0.0),
              255.0
            );
            for (
              var nJitterR1 = nRefineStart;
              nJitterR1 <= nRefineEnd;
              nJitterR1++
            ) {
              InpRmp[RC * 2 + 1] = Math.min(
                Math.max(InpRmp0[RC * 2 + 1] + nJitterR1 * Fctrs[RC], 0.0),
                255.0
              );
              Eq = MkWkRmpPts(
                Eq,
...
function MkWkRmpPts(
  ...
  for (var j = 0; j < 3; j++) {
    for (var k = 0; k < 2; k++) {
      ...
      _OutRmpPts[j * 2 + k] =
        _InpRmpPts[j * 2 + k] + Math.floor(_InpRmpPts[j * 2 + k] / Fctrsr[j]);
...
function BldClrRmp(_Rmp, _InpRmp, dwNumPoints, channel) {
  ...
  if (dwNumPoints % 2) _Rmp[channel * 16 + dwNumPoints] = 1000000.0; // for 3 point ramp; not to select the 4th point as min
  for (var e = 1; e < dwNumPoints - 1; e++)
    _Rmp[channel * 16 + e] = Math.floor(
      (_Rmp[channel * 16] * (dwNumPoints - 1 - e) +
        _Rmp[channel * 16 + dwNumPoints - 1] * e +
...
function BldRmp(_Rmp, _InpRmp, dwNumPoints) {
  for (var j = 0; j < 3; j++) BldClrRmp(_Rmp, _InpRmp, dwNumPoints, j);
}
...
function Refine(
  ...
  for (var i = 0; i < _NmrClrs; i++) {
    for (var r = 0; r < dwNumPoints; r++) {
      ...
      var DistB = Rmp[BC * 16 + r] - Blk[i * 4 + BC];
      RmpErr[r * 64 + i] =
        DistG * DistG * fWeightGreen + DistB * DistB * fWeightBlue;
    }
  }
  ...
  for (var i = nRefineStart; i <= nRefineEnd; i++) {
    for (var j = nRefineStart; j <= nRefineEnd; j++) {
      ...
      InpRmp[RC * 2 + 0] = Math.min(
        Math.max(InpRmp0[RC * 2 + 0] + i * Fctrs[RC], 0.0),
        255.0
      );
      InpRmp[RC * 2 + 1] = Math.min(
        Math.max(InpRmp0[RC * 2 + 1] + j * Fctrs[RC], 0.0),
        255.0
      );
  ...
  for (var i = 0; i < _NmrClrs; i++) {
    for (var r = 0; r < dwNumPoints; r++) {
      ...
      var DistB = Rmp[BC * 16 + r] - Blk[i * 4 + BC];
      RmpErr[r * 64 + i] =
        DistR * DistR * fWeightRed + DistB * DistB * fWeightBlue;
    }
  }
  ...
  for (var i = nRefineStart; i <= nRefineEnd; i++) {
    for (var j = nRefineStart; j <= nRefineEnd; j++) {
      InpRmp[GC * 2 + 0] = Math.min(
        Math.max(InpRmp0[GC * 2 + 0] + i * Fctrs[GC], 0.0),
        255.0
      );
      InpRmp[GC * 2 + 1] = Math.min(
        Math.max(InpRmp0[GC * 2 + 1] + j * Fctrs[GC], 0.0),
        255.0
      );
  ...
  for (var i = 0; i < _NmrClrs; i++) {
    for (var r = 0; r < dwNumPoints; r++) {
      ...
      var DistG = Rmp[GC * 16 + r] - Blk[i * 4 + GC];
      RmpErr[r * 64 + i] =
        DistR * DistR * fWeightRed + DistG * DistG * fWeightGreen;
    }
  }
  ...
  for (var i = nRefineStart; i <= nRefineEnd; i++) {
    for (var j = nRefineStart; j <= nRefineEnd; j++) {
      InpRmp[BC * 2 + 0] = Math.min(
        Math.max(InpRmp0[BC * 2 + 0] + i * Fctrs[BC], 0.0),
        255.0
      );
      InpRmp[BC * 2 + 1] = Math.min(
        Math.max(InpRmp0[BC * 2 + 1] + j * Fctrs[BC], 0.0),
        255.0
      );
  ...
  for (var j = 0; j < 3; j++)
    for (var k = 0; k < 2; k++) _OutRmpPnts[j * 2 + k] = InpRmp[j * 2 + k];

  return bestE;
}

function ClstrErr(_Blk, _Rpt, _Rmp, _NmbClrs, _blcktp, _ConstRamp, _pfWeights) {
  var fError = 0.0;
...
function CompBlock1X(
  ...
  for (var i = 0; i < dwBlockSize; i++) fBlk[i] = _Blk[i] / 255.0;

  var err = CompBlock1XF(
    fBlk,
    dwBlockSize,
    nEndpoints,
    pcIndices,
...
function Clstr1(
  ...
  for (var i = 0; i < _NmbrClrs; i++) pcIndices[i] = 0;
  ...
  for (var i = 0; i < nNumPoints; i++) alpha[i] *= OverIntFctr;
...
function CompBlock1(
  ...
  if (bFixedRampPoints) {
    for (var i = 0; i < _Nmbr; i++) {
      if (new_p != fBlk[i]) {
        new_p = fBlk[i];
        if (new_p <= 1.5 / 255) N0s++;
        else if (new_p >= 253.5 / 255) N1s++;
        else {
        ...
        dwUniqueValues > 0 &&
        afUniqueValues[dwUniqueValues - 1] == new_p
      )
    ...
    if (dwUniqueValues <= 2) {
      ...
      } else if (dwUniqueValues == 1) {
        // if 1, add another one
        Ramp[0] = Math.floor(afUniqueValues[0] * (IntFctr - 1) + 0.5);
        Ramp[1] = Ramp[0] + 1;
      } // if 0, invent them
      else {
    ...
    for (var i = 0; i < _Nmbr; i++) {
      if (new_p != fBlk[i]) {
        ...
        afValueRepeats[dwUniqueValues] = 1;
        dwUniqueValues++;
      } else afValueRepeats[dwUniqueValues - 1] += 1;
    }

    // if number of unique colors is less or eq 2, we've done
    if (dwUniqueValues <= 2) {
      Ramp[0] = Math.floor(afUniqueValues[0] * (IntFctr - 1) + 0.5);
      if (dwUniqueValues == 1) Ramp[1] = Ramp[0] + 1;
      else Ramp[1] = Math.floor(afUniqueValues[1] * (IntFctr - 1) + 0.5);
      fMaxError = 0;
      requiresCalculation = false;
    }
  }
  ...
  if (requiresCalculation) {
    ...
    if (
      ...
      m_step = 1;
      gbl_err = MAX_ERROR;
      for (var i = 0; i < dwUniqueValues; i++) afUniqueValues[i] *= IntFctr - 1;
...
function RmpSrch1(
  ...
  for (var i = 0; i < _NmbrClrs; i++) {
    ...
    if ((del = _Blk[i] - _min_ex) <= 0) v = _min_ex;
    else if (_Blk[i] - _max_ex >= 0) v = _max_ex;
    else v = Math.floor((del + step_h) * rstep) * step + _min_ex;
...
function Refine1(
  ...
  do {
    ...
    for (
      bestmode = -1, mode = 0;
      mode < m_nRefinementStepsAlpha * m_nRefinementStepsAlpha;
      mode++
    ) {
...
function BldRmp1(_Rmp, _InpRmp, nNumPoints) {
  ...
  for (var e = nNumPoints; e < MAX_POINTS; e++) _Rmp[e] = 100000.0;

  _Rmp[0] = _InpRmp[0];
  _Rmp[1] = _InpRmp[1];
  for (var e = 1; e < nNumPoints - 1; e++)
    _Rmp[e + 1] =
      (_Rmp[0] * (nNumPoints - 1 - e) + _Rmp[1] * e) / (nNumPoints - 1);
}
/*--------------------------------------------------------------------------------------------
...
function GetRmp1(
  ...
  if (
    (!bFixedRampPoints && _ramp[0] <= _ramp[1]) ||
    (bFixedRampPoints && _ramp[0] > _ramp[1])
  ) {
    var t = _ramp[0];
    _ramp[0] = _ramp[1];
...
function EncodeAlphaBlock(compressedBlock, nEndpoints, nIndices) {
  ...
  for (var i = 0; i < BLOCK_SIZE_4X4; i++) {
    if (i < 5) compressedBlock[0] |= (nIndices[i] & 0x7) << (16 + i * 3);
    else if (i > 5)
